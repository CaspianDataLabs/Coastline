...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=coastline tests/
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 coastline tests
...
check-safety:
	poetry check
	poetry run bandit -ll --recursive coastline tests
